...
public class Camera {
	...
	public float[] smoothRotation() {
		_current.logistic(_currentDirectionTime, _currentPositionTime,
				_desired, _start);
		_currentPositionTime += TIMESTEP;
		_currentDirectionTime += TIMESTEP;
		Matrix.setLookAtM(_mViewMatrix, 0, _current.getLookX(),
				_current.getLookY(), _current.getLookZ(), _current.getX(),
				_current.getY(), _current.getZ(), STARTUP.getX(),
				STARTUP.getY(), STARTUP.getZ());
		return _mViewMatrix;
	}
